...
static/bundle.js: $(shell find client -name "*.re" -o -name "*.js" ! -name "*.bs.js")
	./node_modules/.bin/browserifyinc client/App.js -dv --outfile static/bundle.js

static/bundle.min.js: $(shell find client -name "*.re" -o -name "*.js" ! -name "*.bs.js")
	./node_modules/.bin/browserify client/App.js -g [ envify --NODE_ENV production ] -g uglifyify | ./node_modules/.bin/terser --compress --mangle > static/bundle.min.js
